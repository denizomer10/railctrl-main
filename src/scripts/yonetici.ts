...
async function refreshAll(showToast = false): Promise<void> {
  try {
    ...
    const [users, audit, feedback, report, files] = results;

    if (users.status === 'fulfilled') state.users = users.value.users || [];
    else failures.push('personel');
    if (audit.status === 'fulfilled') state.logs = audit.value.logs || [];
    else failures.push('audit kayıtları');
    if (feedback.status === 'fulfilled') state.feedbacks = feedback.value.feedbacks || [];
    else failures.push('geri bildirimler');
    if (report.status === 'fulfilled') state.report = report.value.report || null;
    else failures.push('rapor');
    if (files.status === 'fulfilled') state.files = files.value.files || [];
    else failures.push('dosyalar');

    renderAll();
    await loadSystemRecords();
    state.lastSync = Date.now();
    renderSync();
    if (failures.length) notify(`Yüklenemeyen veriler: ${failures.join(', ')}`);
    else if (showToast) notify('Veriler yenilendi');
  } catch (e) {
...
async function createUser(): Promise<void> {
  ...
  try {
    await api('/api/admin/users', {
      method: 'POST',
      body: JSON.stringify({ name, nickname, password, role, gorevi }),
    });
    closeCreate();
